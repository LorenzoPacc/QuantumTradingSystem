...
class QuantumTraderUltimateFinal:
    ...
    def check_emergency_stop(self):
        """Stop trading se drawdown critico"""
        if self.emergency_stop_triggered:
            return True
            
        current_balance = self.get_account_balance()
        emergency_threshold = self.config.TRADING_CONFIG["emergency_stop_pnl"]
        
        # Calcola drawdown dal peak
        if current_balance < self.performance_metrics["peak_balance"]:
            drawdown = self.performance_metrics["peak_balance"] - current_balance
            if drawdown >= abs(emergency_threshold):
                self.trigger_emergency_stop(f"Drawdown critico: ${drawdown:.2f}")
                return True
        
        # Aggiorna peak balance
        self.performance_metrics["peak_balance"] = max(
