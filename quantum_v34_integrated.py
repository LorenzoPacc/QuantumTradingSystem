...
class AdaptiveLearning:
    ...
    def _calculate_win_rate_hours(self, hours):
        """Calculate win rate for last N hours"""
    ...
    def _calculate_rr_ratio(self):
        ...
        try:
            if not hasattr(self, 'trades_history') or len(self.trades_history) < 10:
                return 1.0
            
            recent = self.trades_history[-50:]
            avg_win = sum(t.get('pnl', 0) for t in recent if t.get('pnl', 0) > 0) / max(1, sum(1 for t in recent if t.get('pnl', 0) > 0))
            avg_loss = abs(sum(t.get('pnl', 0) for t in recent if t.get('pnl', 0) < 0)) / max(1, sum(1 for t in recent if t.get('pnl', 0) < 0))
    ...
    def calculate_risk_profile(self):
        ...
        try:
            # Get metrics
            wr24 = self._calculate_win_rate_hours(24)
            wr72 = self._calculate_win_rate_hours(72)
            rr = self._calculate_rr_ratio()
            dd = self._calculate_max_drawdown()
            ...
            logging.info(f"💼 RISK PROFILE: {profile} {emoji} (Score:{risk_score:.0f}, WR24:{wr24:.1f}%, RR:{rr:.2f}, DD:{dd:.1f}%)")
            ...
            logging.error(f"Risk profile error: {e}")
            return {'profile': 'MODERATE', 'score': 50, 'metrics': {}}
    ...
    def _calculate_rr_ratio(self):
        ...
        if not losses:
            return 2.0
        return (sum(wins) / len(wins)) / max((sum(losses) / len(losses)), 0.01)
    
    def _close_all_regime(self, reason):
        """Close all positions for regime hibernation"""
        logger.warning(f"🐻 Closing all positions: {reason}")
        for symbol in list(self.positions.keys()):
            try:
                ticker = self.exchange.fetch_ticker(symbol)
                self._exit_position(symbol, ticker['last'], reason)
            except Exception as e:
...
import requests
from adaptive_regime import AdaptiveRiskRegimeEngine
import atexit
import sys
...
class QuantumTradingV34:
    def __init__(self, initial_capital=200.0, dry_run=True):
        ...
        # Fear & Greed
        self.FG_EXTREME_FEAR = 30  # PATCHED
        self.FG_FEAR = 40
  ...
  # FIX: Lower exposure (was 4)
    ...
    def _calculate_win_rate_hours(self, hours=24):
        """Calculate win rate for last N hours"""
        try:
            if not hasattr(self, 'trades_history') or not self.trades_history:
                return 50.0
            ...
            return (wins / len(recent_trades)) * 100 if recent_trades else 50.0
        except Exception as e:
            if hasattr(self, 'logger'):
                self.logger.error(f"Win rate calc error: {e}")
            return 50.0
    ...
    def calculate_risk_profile(self):
        ...
        try:
            ...
            dd = self._calculate_max_drawdown()
            risk_score = 50
            if wr24 < 40: risk_score += 15
            elif wr24 > 60: risk_score -= 15
            if dd > 15: risk_score += 20
            elif dd < 5: risk_score -= 10
            if rr < 0.8: risk_score += 10
            elif rr > 1.5: risk_score -= 10
            if risk_score < 30: profile, emoji = "CONSERVATIVE", "🛡️"
            elif risk_score < 50: profile, emoji = "MODERATE", "⚖️"
            elif risk_score < 70: profile, emoji = "AGGRESSIVE", "⚡"
            else: profile, emoji = "HIGH_RISK", "🔥"
            logging.info(f"💼 RISK PROFILE: {profile} {emoji} (Score:{risk_score:.0f}, WR24:{wr24:.1f}%, RR:{rr:.2f}, DD:{dd:.1f}%)")
            return {'profile': profile, 'score': risk_score, 'metrics': {'win_rate_24h': wr24, 'win_rate_72h': wr72, 'rr_ratio': rr, 'drawdown': dd}}
        except Exception as e:
            logging.error(f"Risk profile error: {e}")
            return {'profile': 'MODERATE', 'score': 50, 'metrics': {}}
    ...
    def step(self):
        ...
        try:
            ...
            rr = self._calculate_rr_ratio()
            dd = self._calculate_max_drawdown()
            mtf_rate = (self.mtf_passed / max(self.mtf_checks, 1)) * 100
            
            risk_profile = self.regime_engine.calculate_risk_profile(
                fear_greed=fear_index, mtf_pass_rate=mtf_rate, volatility=0.03,
                win_rate_24h=wr24, win_rate_72h=wr72, drawdown=dd, rr_ratio=rr
            )
            ...
            logger.info(f"💼 {risk_profile['profile_name']} | {risk_profile['market_regime']}/{risk_profile['strategy_health']}")
            
            if risk_profile.get('close_existing', False):
                logger.warning("🐻 HIBERNATION")
                self._close_all_regime("HIBERNATION")
                time.sleep(120)
        ...
        self.save_state()
        logger.info("Next cycle in 2 minutes...")
        logger.info("=" * 80)
        return total_value
...
def main():
    ...
    try:
        trader = QuantumTradingV34(initial_capital=200.0, dry_run=True)
