...
class QuantumAutoTrader:
    def __init__(self):
        ...
        self.buy_threshold = 2.9    # -12.5% più aggressivo
        self.sell_threshold = 2.0   # -8.3% più aggressivo
    ...
    def auto_trade(self, symbol, analysis):
        ...
        # SELL AUTOMATICO BTC se score molto basso
        if symbol == "BTCUSDT":
            btc_qty = self.portfolio.get(symbol, 0)
            if btc_qty > 0.001 and score < 2.0:
                self.heartbeat(f"🤖 SELL AUTO BTC: Score basso {score:.2f}")
                return self.execute_market_sell(symbol, btc_qty * 0.5)
