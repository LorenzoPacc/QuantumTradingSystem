...
def main():
    ...
    if portfolio_details:
        ...
        # 🔽 SOLO PULSANTE PER COPIARE PORTFOLIO (NIENTE ALTRO)
        portfolio_text = "\n".join([
            f"{item['symbol']}: {item['quantity']:.6f} units | Current: ${item['current_price']:.4f} | Value: ${item['current_value']:.2f} | Cost: ${item['total_cost']:.2f} | P&L: {item['profit_pct']:+.2f}%"
            for item in portfolio_details
        ])
        
        if st.button("📋 Copia Dettaglio Portfolio"):
            st.code(portfolio_text, language="text")
            st.success("✅ Dati copiati! Usa Ctrl+C per copiare dal box sopra")
    ...
    if trade_history:
        ...
        # 🔽 SOLO PULSANTE PER COPIARE STORICO (NIENTE ALTRO)
        trades_text = "\n".join([
            f"{trade.get('timestamp', '')[:16]} | {trade.get('symbol', '')} | {trade.get('action', '')} | {trade.get('quantity', 0):.6f} | ${trade.get('price', 0):.4f} | ${trade.get('amount', 0):.2f}"
            for trade in recent_trades
        ])
        
        if st.button("📋 Copia Storico Trade"):
