...
st.set_page_config(
    page_title="Quantum Trader Live",
    page_icon="🚀",
    layout="wide"
)
...
<style>
    ...
    .extreme-fear { background-color: #ffebee; border-left: 4px solid #f44336; }
    .fear { background-color: #fff3e0; border-left: 4px solid #ff9800; }
</style>
...
def main():
    st.markdown('<h1 class="main-header">🚀 Quantum Trader Live Dashboard</h1>', 
                unsafe_allow_html=True)
    ...
    if fgi < 25:
        ...
        st.sidebar.success("🎯 **STRATEGIA ATTIVA**: Acquisti aggressivi")
        fear_class = "extreme-fear"
    elif fgi < 40:
    ...
    with col1:
        st.markdown(f'<div class="{fear_class}">', unsafe_allow_html=True)
    ...
    if portfolio_details:
        ...
        if fgi < 25:
            st.success("⚡ **EXTREME FEAR DETECTED** - Il sistema dovrebbe iniziare ad acquistare!")
        elif fgi < 40:
            st.info("📈 **FEAR DETECTED** - Buone opportunità di acquisto")
    
    # Storico Trade
    st.markdown("### 📋 Storico Trade (Ultimi 10)")
    ...
    st.sidebar.info(f"**Asset monitorati**: 7")
    st.sidebar.info(f"**Strategia**: Extreme Fear Contrarian")
