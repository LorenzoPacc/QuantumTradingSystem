...
class PerformanceAnalyticsEngine:
    ...
    def _load_trades(self, lookback_days: int) -> List[Dict]:
        ...
        try:
            ...
            # SCHEMA CORRETTO: total_value invece di total
            cursor.execute("""
                SELECT symbol, action, price, quantity, total_value, timestamp, reason
                FROM trades
                WHERE timestamp >= ?
                ORDER BY timestamp
            """, (cutoff_date,))
            ...
            for trade in trades_raw:
                symbol, action, price, quantity, total_value, timestamp, reason = trade
                ...
                if action == 'BUY':
                    positions[symbol] = {
                        ...
                        'entry_total': total_value
                    }
                elif action == 'SELL' and symbol in positions:
                    entry = positions[symbol]
                    pnl = total_value - entry['entry_total']
    ...
    def _calculate_profit_factor(self, trades: List[Dict]) -> float:
        ...
        if gross_loss == 0:
            return 0.0 if gross_profit == 0 else 99.99
    ...
    def print_report(self, metrics: Dict):
        ...
        if metrics['best_symbols']:
            print(f"   Best Symbols:")
            for symbol, pnl in metrics['best_symbols']:
                print(f"      {symbol}: {pnl:+.2f}%")
        if metrics['worst_symbols']:
            print(f"   Worst Symbols:")
            for symbol, pnl in metrics['worst_symbols']:
                print(f"      {symbol}: {pnl:+.2f}%")
