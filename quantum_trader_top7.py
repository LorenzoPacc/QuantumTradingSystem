...
class QuantumTraderTop7:
    """STRATEGIA: BTC + ETH + TOP 5 ALTCOIN"""
    ...
    def __init__(self, initial_balance: float = 200):
        ...
        self.logger.info("🚀 QUANTUM TRADER - TOP 7 CRYPTO STRATEGY")
    ...
    def analyze_extreme_fear(self, symbol: str, asset_tier: str) -> Tuple[str, float, str]:
        """ANALISI con priorità per BTC/ETH"""
        try:
    ...
    def run_cycle(self) -> int:
        ...
        print(f"\n{'='*70}")
        print(f"🎯 CICLO {self.cycle_count} - TOP 7 CRYPTO STRATEGY")
        print(f"{'='*70}")
        ...
        # 🔥 TOP 7 CRYPTO: BTC + ETH + 5 Best Altcoin
        assets = [
            # MAJOR (priorità massima)
            {'symbol': 'BTCUSDT', 'tier': 'MAJOR', 'name': 'Bitcoin'},
            {'symbol': 'ETHUSDT', 'tier': 'MAJOR', 'name': 'Ethereum'},
            
            # TOP ALTCOIN (per market cap e utilità)
            {'symbol': 'SOLUSDT', 'tier': 'ALTCOIN', 'name': 'Solana'},
            {'symbol': 'AVAXUSDT', 'tier': 'ALTCOIN', 'name': 'Avalanche'},
            {'symbol': 'LINKUSDT', 'tier': 'ALTCOIN', 'name': 'Chainlink'},
            {'symbol': 'DOTUSDT', 'tier': 'ALTCOIN', 'name': 'Polkadot'},
            {'symbol': 'MATICUSDT', 'tier': 'ALTCOIN', 'name': 'Polygon'}
        ]
        
        decisions = 0
        max_assets = 5  # Massimo 5 asset contemporaneamente
        ...
        for asset in assets:
            ...
            action, score, reason = self.analyze_extreme_fear(symbol, tier)
            ...
            # TRADING LOGIC
            if action == "BUY" and available_balance > 25 and current_assets < max_assets:
                if symbol not in self.engine.portfolio:
                    # Position sizing intelligente
                    if tier == "MAJOR":
                        # BTC/ETH: allocation più grande
                        if fgi < 25:
                            amount = min(50, available_balance * 0.40)  # 40% per Major in Extreme Fear
                        else:
                            amount = min(40, available_balance * 0.35)
                    else:
                        # Altcoin: allocation standard
                        if fgi < 25:
                            amount = min(35, available_balance * 0.30)
                        else:
                ...
                if profit_data:
                    ...
                    # Soglie vendita diverse per Major vs Altcoin
                    if tier == "MAJOR":
                        # BTC/ETH: hold più lungo
                        sell_profit = 15
                        sell_loss = -8
                    else:
                        # Altcoin: più aggressivo
                        sell_profit = 12
                        sell_loss = -6
                    
                    if pnl > sell_profit or pnl < sell_loss:
                        if self.engine.market_sell(symbol):
                            decisions += 1
                            print(f"   🔴 VENDUTO (P&L: {pnl:.1f}%)")
